...
async function callGeminiParser(text) {
  try {
    ...
    - SHOPPING (clothing, e-commerce, electronics)
    - ENTERTAINMENT (movies, streaming services, events, gaming, bars)
    - BILLS (utilities, rent, mobile recharge, subscriptions)
    - INVESTMENTS (stocks, mutual funds, gold, crypto)
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            category: { type: Type.STRING, enum: ["FOOD", "SHOPPING", "ENTERTAINMENT", "BILLS", "INVESTMENTS", "OTHERS"] },
